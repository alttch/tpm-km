...
if [ ! $PCRS ] || [ ! $ADDRS ] || [ ! $KEYFILE ]; then
  print_err "TPM params not loaded, config missing?"
  exit 11
fi

while [ ! $PIN ]; do
  PIN=$(dialog --clear --insecure --passwordbox "Define PIN:" 10 30 3>&1 1>&2 2>&3)
  PIN2=$(dialog --clear --insecure --passwordbox "Verify PIN:" 10 30 3>&1 1>&2 2>&3)
  [ ! $PIN ] || [ ! $PIN2 ] && exit 0
...
for pcr in `echo ${PCRS} | tr "," "\n" | tr " " "\n"|sort -n|uniq`; do
  VAL=`tpm2_pcrlist -L sha256:${pcr}|tail -1|awk '{ print $3 }'`
...
for i in $(seq 0 $END); do
  ...
  tpm2_createpolicy -P -L sha256:${PCRS[$i]} -f $pfile
  if [ $? -ne 0 ]; then
    print_err "Unable to create policy for ${PCRS[$i]}"
    exit 2
  fi
  tpm2_nvrelease -x ${ADDRS[$i]} -a 0x40000001 > /dev/null 2>&1
  tpm2_nvdefine -x ${ADDRS[$i]} -a 0x40000001 -L $pfile -s 255 -t "policyread|policywrite"
  if [ $? -ne 0 ]; then
    print_err "Unable to define nv for ${ADDRS[$i]}"
    rm -f $pfile
    exit 3
  fi
  echo -n $CHUNK | tpm2_nvwrite -x ${ADDRS[$i]} -a ${ADDRS[$i]} -L sha256:${PCRS[$i]}
  if [ $? -ne 0 ]; then
